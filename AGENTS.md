...
- **AmneziaWG 1.5/3.1 в APK:** общий Go-пакет [awg-shim/](awg-shim/) собирается gomobile в `awgshim.aar` ([build-awg-shim-android.ps1](scripts/build-awg-shim-android.ps1)). Kotlin `VpnService` запускает его в том же процессе на `127.0.0.1:2081` до libbox; Rust подменяет AWG outbound на локальный SOCKS5. Поэтому остаются sing-box RU/LAN/per-app split и Clash API статистика. UDP-сокеты AWG исключаются из VPN-петли через общий gomobile-интерфейс → `VpnService.protect(fd)`. Конфиг AWG сохраняется вместе с быстрым подключением, поэтому sticky/Always-on, плитка и виджеты умеют восстановить AWG-сеанс. Поля AWG 3.1 обрабатывает закреплённый `amneziawg-go/v3 v3.1.20260814`.
...
- Инструменты: `scripts/setup-android.ps1`; сборка libbox: `scripts/build-libbox-android.ps1`; сборка AWG 3.1 shim: `scripts/build-awg-shim-android.ps1`; APK без Windows Developer Mode: `scripts/build-android.ps1`.
- `libbox.aar`, `awgshim.aar`, JNI `.so` и Gradle build outputs в git не хранятся. У двух gomobile AAR совпадают служебные `go.Seq`-классы; AWG build-скрипт удаляет свою дублирующую копию и использует копию libbox, иначе D8 падает на duplicate class.
...
- **awg-shim** (свой, [awg-shim/](awg-shim/)): Go 1.26 + `amneziawg-go/v3 v3.1.20260814` + gvisor go-ветка. Общая библиотека используется desktop-командой `cmd/awg-shim` и Android gomobile AAR. Windows-сборка: [scripts/build-awg-shim.ps1](scripts/build-awg-shim.ps1) → `binaries/awg-shim-x86_64-pc-windows-msvc.exe`; Android: [scripts/build-awg-shim-android.ps1](scripts/build-awg-shim-android.ps1) → игнорируемый `app/libs/awgshim.aar`. Портативный Go для desktop-сборки — `%LOCALAPPDATA%\unigate-tools\go`.
