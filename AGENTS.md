...
- [x] Обход LAN (`ip_is_private`) и RU (`geoip-ru.srs` rule-set + домены `.ru/.рф/.su`) → direct. При `bypass_lan` в `tun.route_exclude_address` всегда идут RFC1918 + `100.64.0.0/10` + `169.254.0.0/16`. На Windows при `vpn_compatibility` до старта TUN дополнительно обнаруживаем конкретные IPv4-маршруты активных OpenVPN/WireGuard/корпоративных VPN-адаптеров. Default/half-default и префиксы шире `/8` отбрасываются: обычный интернет по-прежнему перекрывает UniGate, а за вторым VPN остаются только его корпоративные сети.
...
- **Заметки:** geosite-ru официального нет → RU по geoip-ru (IP) + `.ru/.рф/.su` (домены). `geoip-ru.srs` в `binaries/` в git не коммитим. Fetch-скрипты теперь всегда скачивают свежую базу через temp + проверку magic/size; при подключении `prepare_geoip()` раз в сутки обновляет кэш в app config dir, а при офлайне/ошибке берёт прежний кэш или bundled-копию. Split — только sing-box в TUN, не legacy AmneziaWG.
...
- [x] Контейнеры `amnezia-awg`/`wireguard` → движок **amneziawg.exe** (amneziawg-windows-client 3.1.0). Импорт awg-контейнера: берём `awg.last_config.config` (готовый `.conf`), подставляем DNS (`dns1`/`dns2`), добавляем `MTU` из `last_config.mtu`. Модель `Outbound::AmneziaWg{config,server,port}`; ветка движка в [connection.rs](src-tauri/src/connection.rs) (`/installtunnelservice`/`/uninstalltunnelservice`). Формат 3.1 с `HeaderProtectionKey`, padding/timing ranges, `RandomTrailers` и `DisableCookies` сохраняется без потерь.
...
- **Windows + доменный AWG Endpoint:** перед запуском TUN домен резолвится системным DNS, выбранный IP закрепляется в рабочем конфиге шима на эту сессию и добавляется в `tun.route_exclude_address` вместе с direct-правилом. На следующем подключении DNS проверяется снова. Если IP определить не удалось, подключение прерывается до создания TUN, чтобы не зациклить трафик.
- **Windows release-сборка:** `tauri.windows.conf.json` пересобирает `awg-shim` перед сборкой фронтенда. Нельзя упаковывать старый бинарник из `src-tauri/binaries`: версия `amneziawg-go v0.2.19` отвергает диапазон `PersistentKeepalive = 25-35` из AWG 3.1, из-за чего не запускаются даже профили с IP endpoint. Интеграционный Go-тест проверяет передачу диапазона до `device.IpcSet`.
...
- **⚠️ Зависимости Go:** shim обновлён на модуль `amneziawg-go/v3` **v3.1.20260814**. Он поддерживает прежние AWG-поля и параметры 3.1; конфиговые booleans `on/off` нормализуются в UAPI `true/false`, а `HeaderProtectionKey` переводится из base64 в hex.
...
- **macOS:** awg-shim собирается в `fetch-awg-macos.sh` и бандлится как sidecar. AmneziaWG идёт через sing-box, поэтому RU/LAN/per-app split и Clash API статистика доступны как на Windows. `awg-quick` остаётся fallback, если shim отсутствует. Связка ждёт реального теста на Mac.
...
- [ ] **Отложено (по решению пользователя):** локализация RU/EN (муторно, юзер RU); автообновление ядра sing-box.
- [x] **Обновления приложения:** sidebar проверяет последний GitHub Release. Windows скачивает и запускает подписанный Tauri updater-пакет; macOS и Android открывают страницу релиза (на macOS автоматическая замена отключена до Developer ID/notarization). CI подписывает Windows NSIS отдельным updater-ключом и публикует `latest.json`.
...
**⚠️ macOS TCC — критичная находка (подтверждено логом):** папки `~/Desktop`/`~/Documents`/`~/Downloads` закрыты TCC. Процесс sing-box (даже от root, запущенный приложением — другая цепочка ответственности TCC) **не может читать оттуда data-файлы**: `open .../binaries/geoip-ru.srs: operation not permitted` → ядро падает `FATAL initialize router` → TUN не поднимается. Проект лежал на Рабочем столе → RU-обход (`bypass_ru`, грузит `geoip-ru.srs`) ронял TUN. **Фикс:** общий `prepare_geoip()` для всех desktop-ОС держит проверенную копию geoip в app config dir (`~/Library/Application Support/com.unigate.app/` — под TCC НЕ попадает) и подставляет этот путь в конфиг ([connection.rs](src-tauri/src/connection.rs)). `running-config.json`/`sing-box.log` изначально там же — читаются/пишутся нормально.

**Сделано на маке (компилируется; ✅ ГОТОВО помечаем после проверки пользователем):**
- [x] **AmneziaWG устойчивость сети (1.1.2):** при отсутствии системного IPv6-маршрута из рабочей копии профиля убираются IPv6 `AllowedIPs`. Системный DNS-этап `awg-quick` выполняется best-effort: ошибки/пустые ответы `networksetup` для отдельных сервисов (включая USB Ethernet) больше не откатывают уже поднятый `utun` и IPv4-маршруты. Сохранённый профиль не меняется.
...
- [~] **AmneziaWG под macOS:** legacy `awg-quick` реально подключается (подтверждено пользователем). Основной путь теперь `awg-shim` → локальный SOCKS5 → sing-box, что даёт RU/LAN/per-app split и Clash API статистику. [scripts/fetch-awg-macos.sh](scripts/fetch-awg-macos.sh) собирает shim под текущую Apple-архитектуру; [tauri.macos.conf.json](src-tauri/tauri.macos.conf.json) бандлит его как sidecar. При отсутствии shim сохраняется рабочий fallback на `amneziawg-go v3.1.20260814` + `awg/awg-quick v3.1.20260812`, но без split/статистики. Новая shim-связка ждёт реального теста на Mac. После установки неподписанного `.app` один рекурсивный `xattr -dr com.apple.quarantine /Applications/UniGate.app` охватывает и вложенные движки.
...
- **Пустой DNS-ответ USB-Ethernet на macOS:** некоторые сервисы `networksetup` (проверено по логу с `AX88179B`) успешно возвращают пустую строку для `-getdnsservers`/`-getsearchdomains`. Upstream `[[ -n $response ]] && assignment` при `set -e` из-за этого аварийно завершал `awg-quick up` и trap откатывал уже поднятый `utun`. [awg-quick-bash3.patch](scripts/awg-quick-bash3.patch) теперь сохраняет `Empty` и продолжает запуск. Сообщения `route ... -inet6 ... Network is unreachable` на IPv4-only интерфейсе сами по себе не инициируют откат: macOS `route` в этом случае возвращает успешный status.
...
- Android использует не sidecar-процесс, а `sing-box v1.14.0` как `libbox.aar` внутри нативного Kotlin `VpnService`.
...
- **AmneziaWG 1.5/3.1 в APK:** общий Go-пакет [awg-shim/](awg-shim/) и sing-box libbox собираются одним вызовом gomobile в единый `libbox.aar` ([build-libbox-android.ps1](scripts/build-libbox-android.ps1)). Kotlin `VpnService` запускает shim в том же процессе на `127.0.0.1:2081` до libbox; Rust подменяет AWG outbound на локальный SOCKS5. Поэтому остаются sing-box RU/LAN/per-app split и Clash API статистика. UDP-сокеты AWG исключаются из VPN-петли через общий gomobile-интерфейс → `VpnService.protect(fd)`. Конфиг AWG сохраняется вместе с быстрым подключением, поэтому sticky/Always-on, плитка и виджеты умеют восстановить AWG-сеанс. Поля AWG 3.1 обрабатывает закреплённый `amneziawg-go/v3 v3.1.20260814`.
...
- Инструменты: `scripts/setup-android.ps1`; совместная сборка libbox + AWG 3.1 shim: `scripts/build-libbox-android.ps1`; APK без Windows Developer Mode: `scripts/build-android.ps1`.
- `libbox.aar`, JNI `.so` и Gradle build outputs в git не хранятся. sing-box и awg-shim обязаны находиться в одном gomobile AAR и одном Go-runtime: два отдельных AAR делят Java-класс `go.Seq`, но имеют несовместимые native runtime и приводят к `NoClassDefFoundError`/ошибке инициализации Awgshim на устройстве.
...
- **Release pipeline Android:** tag-workflow `.github/workflows/build-installers.yml` собирает отдельный ARM64 debug/self-signed APK на Windows runner и публикует файл `UniGate_<version>_android_arm64.apk` рядом с MSI/NSIS/DMG. Ключ берётся только из обязательного GitHub Secret `ANDROID_DEBUG_KEYSTORE_BASE64`, затем подпись готового APK и публичный SHA-256 сертификата сверяются через `apksigner`; без Secret или при подмене ключа сборка падает. Постоянная цепочка начинается с 1.1.0. Прежний Actions cache был изолирован по тегам и молча создавал разные ключи, из-за чего Android требовал удалить старую версию. Доверенной publisher/Play-подписи нет, поэтому Android/Play Protect/HyperOS предупреждает об установке из неизвестного источника. `RELEASE_NOTES.md` используется как оформленное описание релиза.
- **Восстановление после блокировки/Doze (август 2026):** `VpnService` остаётся sticky, но при системном рестарте с `intent=null` восстанавливает последний сохранённый конфиг только если VPN не отключали вручную либо включён системный Always-on. Отдельный persistent-флаг `should_reconnect` защищает от самопроизвольного запуска после Disconnect. Native status теперь берётся из живого состояния сервиса, а не только из сохранённого `ON`. Default-network monitor использует упорядоченный `onLinkPropertiesChanged` и переданный `LinkProperties` вместо гонки `getLinkProperties()` из `onAvailable`; текущая физическая сеть также передаётся Android через `setUnderlyingNetworks`, поэтому libbox получает корректный интерфейс после Wi-Fi/LTE handoff и пробуждения.
...
- **TUN-конфиг sing-box 1.14 (рабочая схема, проверена `sing-box check`):** новый формат DNS-серверов (`{type,tag,server}`, не legacy `address`); в `route` обязателен `default_domain_resolver` (иначе deprecation-fatal); `tun` inbound — `address`/`auto_route`/`strict_route`/`stack:"system"` (`strict_route:false` только на Windows при одновременно включённых `bypass_lan` + `vpn_compatibility`); в `route.rules` — `{action:"sniff"}` и `{protocol:"dns",action:"hijack-dns"}`; нужен `direct` outbound. В TUN-режиме системный прокси НЕ ставим.
...
- **Апгрейд sing-box 1.13.14 → 1.14.0 (сентябрь 2026):** из нашей схемы затронут ровно один пункт — `dns.independent_cache` объявлен deprecated (удаление в 1.16), кэш и так раздельный по тегу сервера ⇒ поле убрано из [config.rs](src-tauri/src/config.rs) `generate_tun`. Остальные breaking changes 1.14 (`tls.acme` → `certificate_provider`, `store_rdrc` → `store_dns`, legacy address-filter в DNS-правилах, `download_detour` у remote rule-set) нас не касаются — мы их не используем. Все генерируемые схемы (proxy, TUN plain/RU+LAN+per-app, hysteria2/vless-reality/trojan-ws/tuic/ss) перепроверены `sing-box check` на 1.14.0 — чисто, без deprecation-предупреждений. Android-libbox собирается из того же тега; gomobile-CLI поднят до v0.1.13 (как в Makefile релиза) и закреплён stamp-файлом, чтобы апгрейд не собирал AAR старым биндером. **Android:** libbox 1.14 расширил `PlatformInterface` (13 новых методов, `SystemCertificates` убран) и `CommandServerHandler` (`triggerNativeCrash`, `connectSSHAgent`), а `TunOptions.getDNSServerAddress()` теперь отдаёт `StringIterator` вместо одного адреса — [UniGateVpnService.kt](src-tauri/gen/android/app/src/main/java/com/unigate/app/vpn/UniGateVpnService.kt) обновлён: заглушки повторяют семантику `platformInterfaceStub` из libbox, DNS из ядра прокидывается списком. **dns_mode НЕ трогаем:** новая опция tun по умолчанию (`hijack`) на Windows и macOS воспроизводит поведение 1.13 байт-в-байт (проверено по коду sing-tun 0.8.11 vs 0.9.0-beta.4: `luid.SetDNS` на LUID самого tun-адаптера и WFP-фильтр DNS под `strict_route` были и раньше).
...
- **sing-box: v1.14.0** (Windows amd64). Скрипт загрузки: [scripts/fetch-singbox.ps1](scripts/fetch-singbox.ps1). Бинарник лежит в `src-tauri/binaries/sing-box-<target-triple>.exe`, **в git не коммитится** (gitignore) — тянется скриптом. При апгрейде версии сверять схему конфига.
- **awg-shim** (свой, [awg-shim/](awg-shim/)): Go 1.26 + `amneziawg-go/v3 v3.1.20260814` + gvisor go-ветка. Общая библиотека используется desktop-командой `cmd/awg-shim` и на Android внутри объединённого `libbox.aar`. Windows-сборка: [scripts/build-awg-shim.ps1](scripts/build-awg-shim.ps1) → `binaries/awg-shim-x86_64-pc-windows-msvc.exe`; Android: [scripts/build-libbox-android.ps1](scripts/build-libbox-android.ps1). Портативный Go для desktop-сборки — `%LOCALAPPDATA%\unigate-tools\go`.
- **wintun: v0.14.1** (amd64) — `src-tauri/binaries/wintun.dll`, в git не коммитится.
- **AmneziaWG-движок: amneziawg-windows-client v3.1.0** — `amneziawg.exe` + `awg.exe`, извлекаются из MSI, кладутся в `src-tauri/binaries/`, в git не коммитятся. Версия закреплена stamp-файлом, поэтому fetch-скрипт заменяет старый движок при обновлении. Модель как у WireGuard: `amneziawg.exe /installtunnelservice <.conf>` поднимает тоннель Windows-сервисом (TUN+IP+роуты+DNS+обфускация — всё внутри), `/uninstalltunnelservice <name>` снимает. Нужен админ (есть из 7a).
