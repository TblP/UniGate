...
- [x] **AmneziaWG устойчивость сети (1.1.0):** ошибки/пустые ответы `networksetup` для отдельных сервисов (включая USB Ethernet) больше не роняют `awg-quick`; при отсутствии системного IPv6-маршрута из рабочей копии профиля убираются IPv6 `AllowedIPs`, чтобы macOS не откатывал уже поднятый IPv4-туннель. Сохранённый профиль не меняется.
