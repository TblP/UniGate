...
Рабочий контекст проекта для Claude Code. Держим этот файл в актуальном состоянии: при смене решений — правим здесь.
...
- [x] **Экспорт** профиля в share-ссылку (socks/http → JSON, AmneziaWG → .conf) — [export.rs](src-tauri/src/export.rs), команда `export_profile`, кнопка «Поделиться»; round-trip юнит-тесты
...
- [x] Обход LAN (`ip_is_private`) и RU (`geoip-ru.srs` rule-set + домены `.ru/.рф/.su`) → direct
...
- [~] **AmneziaWG под macOS** (реализовано, ждёт теста на маке): движок = `amneziawg-go` (userspace-датапас, utun) + `amneziawg-tools` (`awg` + `awg-quick`). `amneziawg-apple` НЕ подходит — это готовое отдельное приложение (GUI + NetworkExtension), не библиотека. `connect_amneziawg` в [connection.rs](src-tauri/src/connection.rs) ветвится по ОС: Windows → `amneziawg.exe /installtunnelservice`; macOS → `run_awg_quick("up"/"down", conf)` (bash awg-quick от root, PATH+`WG_QUICK_USERSPACE_IMPLEMENTATION=amneziawg-go`). Тот же `.conf`, что и на Windows. Требует root (как TUN). Prebuilt-бинарников нет → собираются из исходников: [scripts/fetch-awg-macos.sh](scripts/fetch-awg-macos.sh) (нужны Go+make+git), кладутся в `src-tauri/binaries/` (`amneziawg-go`/`awg`/`awg-quick`). **Статистики нет** (как и на Windows — нет Clash API). Бандлинг в prod-`.dmg` пока не настроен (движок опционален; при отсутствии — понятная ошибка «движок не найден»).
