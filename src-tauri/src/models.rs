...
pub struct Routing {
    ...
    /// Сохраняет его конкретные маршруты и освобождает системный DNS из TUN,
    /// но не отдаёт второму VPN обычный интернет. Требует `bypass_lan`, по умолчанию выключена.
    #[serde(default)]
